...
void ParsingRoutes::verify_route()
{
	if(_routeLoc.empty())
	{
		Parsing::error_message("Route has no name");
	}

	if(_rootPath.empty())
	{
		Parsing::error_message("Route " + _routeLoc + " has no root");
    }
    if(_autoIndex != 0 && _autoIndex != 1)
	{
    	Parsing::error_message("Autoindex must be 1 or 0");
	}
    if(_configMethods.empty())
	{
    	Parsing::error_message("Allowed methods are not set");
	}
	if(_upload_status != 1 && _upload_status != 0)
	{
		Parsing::error_message("Upload must be 1 or 0");
	}
	if((_cgi_path.empty() || _cgi_ext.empty()) && !(_cgi_path.empty() || _cgi_ext.empty()))
	{
		Parsing::error_message("CGI Error, extention or path empty\n Required both be set or left empty");
	}
	 ...
	 _redirection()
{
    ...
    while (std::getline(stream, line))
    {
        ...
        if (pos <= 0)
            Parsing::error_message("Invalid config line: " + line);
		...
		if (key.empty() || value.empty())
			Parsing::error_message("Key or value is empty in line: " + line);
...
void ParsingRoutes::assign(const std::string &key, const std::string &value)
{
    ...
    if (it != _setters.end())
        ...
        Parsing::error_message("Unknown route config key: " + key);
}